...
<img src="./scope-closures-fig2.png" width="500">
...
但如果 _引擎_ 在代码中发现一个 `eval(..)` 或 `with`，它实质上就不得不 _假定_ 自己知道的所有的标识符的位置可能是无效的，因为它不可能在词法分析时就知道你将会向`eval(..)`传递什么样的代码来修改词法作用域，或者你可能会向`with`传递的对象有什么样的内容来创建一个新的将被查询的词法作用域。
